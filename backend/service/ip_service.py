from abc import ABC, abstractmethod

from config.redis_setup import redis_container

import time
import uuid

class IpService(ABC) :
    
    @abstractmethod
    def regist_fail_ip(ip : str, user_id : str) : 
        pass
    
    @abstractmethod
    def regist_block_ip(ip : str) : 
        pass
    
    @abstractmethod
    def exists_block_ip(ip : str) :
        pass

    @abstractmethod
    def delete_ip(ip : str) :
        pass

    
class RedisManageIpServiceImpl(IpService) :
    ''' keyPattern
        - blockIpKey = f"block_ip:{ip_address.client_ip}:차단일시"
        - countIpKey = f"fail_ip:{ip}:{id}:등록일시"
    '''
    
    async def regist_fail_ip(self, ip:str, user_id : str) -> int:
        '''실패 IP 등록
            - 회원ID 수집을 위해 zset 사용
        '''
        
        now = int(time.time())
        key = f"fail_ip:{ip}"
        
        member = f"{user_id}:{now}:{uuid.uuid4().hex}"
        
        pipe = redis_container.ip.pipeline()
        pipe.zadd(key, {member : now}) # 1회 추가
        pipe.zremrangebyscore(key, 0, now - 60) # 1분 이전 제거
        pipe.zcard(key)             # 현재 횟수
        pipe.expire(key, 60)        # TTL 유지
        _, _, count, _ = await pipe.execute()
        
        return count
    
    
    
    # 순서 보장을 위한 Lua 스크립트
    BLOCK_IP_LUA = """
    redis.call("SET", KEYS[1], 1, "EX", ARGV[1])
    redis.call("DEL", KEYS[2])
    return 1
    """
    
    async def regist_block_ip(self, ip : str) :
        '''IP 차단(LuaScript)
            1) 블록_IP 등록
            2) FAIL_IP 모두 삭제
        '''
        block_key = f"block_ip:{ip}"
        fail_key = f"fail_ip:{ip}"
        ttl = 3600  # 1시간 차단
        
        block_ip_script = redis_container.ip.register_script(self.BLOCK_IP_LUA)
        
        await block_ip_script(keys=[block_key, fail_key], args=[ttl])
    
    # IP가 Redis에 Block IP로 등록되어 있는지?
    async def exists_block_ip(self, ip: str) -> bool:
        '''
        IP가 메모리 상의 Block IP에 등록되어 있는지?
        '''
        
        pattern = f"block_ip:{ip}:*"
        cursor = 0
        
        while True:
            cursor, keys = await redis_container.ip.scan(cursor=cursor, match=pattern, count=100)
            if keys:  # 하나라도 발견
                return True
            if cursor == 0:
                break

        return False
    
    
    
    # IP 삭제 - Redis는 패턴으로
    async def delete_ip(self, ip:str) :
        await redis_container.ip.delete(f"fail_ip:{ip}")
    
    
    
    async def debug_fail_by_ip(self, ip: str):
        '''IP가 COUNT_IP Redis에 등록되어 있는지? ID 별 합산 제공(DEBUG 용 메소드)'''
        key = f"fail_ip:{ip}"
        members = await redis_container.ip.zrange(key, 0, -1)

        stats = {}

        for m in members:
            if isinstance(m, bytes):
                m = m.decode()

            user_id = m.split(":", 1)[0]
            stats[user_id] = stats.get(user_id, 0) + 1

        return stats
    